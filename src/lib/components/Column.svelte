...
<style>
  ...
  /**
   * 펼친 카드가 옆 레인에 덮이지 않게 합니다.
   *
   * 아래 .zone의 clip-path는 세로만 자르려고 쓴 것인데, clip-path는 쌓임
   * 맥락을 함께 만듭니다. 그래서 카드에 z-index를 아무리 크게 줘도 그 값이
   * 레인 밖으로 나가지 못하고, 레인끼리는 DOM 순서대로 그려집니다 — 뒤
   * 레인이 앞 레인의 펼친 카드를 통째로 덮습니다.
   *
   * 맨 오른쪽 레인만 멀쩡해 보이던 이유가 이것입니다. 그 레인은 왼쪽으로
   * 펼치는데 자기가 마지막 순서라 덮을 것이 없었습니다.
   *
   * 카드가 아니라 레인을 올립니다. 카드의 z-index는 갇혀 있지만 레인은
   * 형제들과 같은 층에 있으니 여기서는 통합니다.
   */
