...
<style>
   ...
   */
  /**
   * 입력줄과의 간격을 gap이 아니라 제 margin으로 냅니다.
   *
   * gap은 요소에 붙은 것이 아니라 요소 **사이**에 있습니다. 그래서 slide가
   * 줄여 줄 수 없고, 이 상자가 DOM에서 빠지는 순간 7px이 통째로 사라집니다.
   * 재보니 미끄러짐이 0.03px까지 매끄럽게 내려간 다음 마지막 한 프레임에서
   * 패널이 8px 뚝 떨어졌습니다 — 닫히는 끝에서 한 번 끊기던 것이 이것입니다.
   *
   * margin으로 옮기면 slide가 높이와 함께 이것도 0으로 줄입니다. 빠질 때는
   * 이미 0이라 사라질 것이 없습니다.
   *
   * 안쪽 두 줄(마감·반복) 사이의 간격은 여기 gap이 그대로 냅니다 — 그 둘은
   * 같이 나타나고 같이 사라지므로 어긋날 일이 없습니다.
   */
  .reveal {
    display: flex;
  ...
  form {
    display: flex;
    flex-direction: column;
    /* 간격은 자식이 제 margin으로 냅니다. 위 .reveal 주석을 보세요. */
    gap: 0;
    width: 0;
    min-width: 100%;
  }
