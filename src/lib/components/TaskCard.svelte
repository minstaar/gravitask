...
<style>
  ...
  .card:hover {
    /* 남은 시간과 마감 시각이 한 줄에 들어가면 카드는 커지지 않으므로 이
       여백은 평소에 쓰이지 않습니다. 레인이 좁아 둘이 접힐 때만 값을 합니다 —
       그때 글자가 테두리에 닿는데, 한글은 라틴 문자보다 세로로 꽉 차서 닿는
       정도가 아니라 겹쳐 보입니다. */
    padding-top: 4px;
    padding-bottom: 4px;
    right: auto;
   ...
   *
   * 그래도 레인이 아주 좁으면 한 줄에 못 담을 수 있어 flex-wrap으로 접히게
   * 둡니다. 그때만 예전처럼 자라고, 평소에는 자라지 않습니다.
   */
  .card:hover .when {
    display: block;
  }
