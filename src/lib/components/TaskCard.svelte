...
<style>
  ...
  .card:hover {
    /* 펼치면 세 줄이 되는데 접혔을 때는 한 줄이라 위아래 여백이 필요 없었습니다.
       그대로 두면 글자가 테두리에 닿습니다 — 한글은 라틴 문자보다 글자가 세로로
       꽉 차서 그냥 닿는 정도가 아니라 겹쳐 보입니다. 카드는 아래를 붙잡고 위로
       자라므로, 여백이 늘어도 아래 모서리가 마감 시각이라는 뜻은 그대로입니다. */
    padding-top: 7px;
    padding-bottom: 7px;
    right: auto;
    width: max-content;
