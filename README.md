...
한 줄 입력으로도 됩니다 — `매주 월요일 알고리즘 과제`, `평일마다 스탠드업 오전 10시`,
`매월 1일 월세`, `3일마다 화분 물 주기`. 요일은 붙여 적어도 되고(`매주 월수금 운동`)
나눠 적어도 됩니다 — `매주 월, 수, 금 운동`, `매주 수요일과 금요일 회의`.
...
않으니 기록에 남지 않고, 다음 회차로 굴러갑니다.

이번 주만 다른 날짜나 시각으로 미루려면 `수정`에서 마감만 바꾸면 됩니다.
규칙이 요일·날짜·시각을 따로 기억하고 있어서 다음 회차는 원래 자리로
돌아옵니다 — 매주 월요일 2시 랩미팅을 이번 주만 목요일 4시로 미뤄도 다음
주는 월요일 2시입니다.

앞으로 계속 바꾸려면 반복 칩을 열어 규칙을 고칩니다. 요일은 요일 단추로,
주기는 `매주`·`격주`·`매월`로. 시각은 마감을 새 시각으로 맞춘 뒤 주기 단추를
한 번 더 누르면 그 시각이 규칙의 기준이 됩니다.
...
여기서도 자연어가 통합니다. 다만 **원래 제목 뒤에 덧붙인 부분만** 읽습니다 —
`확률론 과제`를 `확률론 과제 내일 3시`로 고치면 마감만 내일 15시로 옮겨가고
제목은 그대로입니다. 앞부분을 건드리지 않는 것은 `내일 회의 준비` 같은 제목이
열자마자 잘려 나가지 않게 하기 위해서입니다. 열었다가 그냥 저장하면 아무것도
바뀌지 않습니다. 제목을 통째로 다시 쓰면 그때는 새로 적은 것으로 보고 전체를
읽습니다.
