...
```
        업무            생활
      ┌──────────────┬──────────────┐
      │ [학회 26일]  │              │   하루 이상 남은 일 — 순서만 전달
      │ [논문  9일]  │ [월세 17일]  │
  24H ├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┤
  12h │              │ [건강검진]   │   24시간 안쪽 — 실제 시간 눈금 위
  DUE ┝━━━━━━━━━━━━━━━━━━━━━━━━━━━━┥
      │ [확률론]     │              │   기한 지남 — 밀릴수록 깊이 가라앉음
      └──────────────┴──────────────┘
```
...
> **macOS에서 메뉴 막대 아이콘이 안 보이면 앱을 다시 실행하세요.** 메뉴 막대는
> 자리가 모자라면 아이콘을 말없이 숨깁니다. Spotlight나 응용 프로그램 폴더에서
> Gravitask를 다시 열면 위젯이 나타납니다 — 앱이 두 개 뜨지는 않습니다.
...
주소는 사실상 비밀번호라 OS 자격 증명 저장소에 보관하고, 앱의 화면 쪽으로는
넘기지 않습니다.
...
- **서명 인증서가 없어** 첫 실행에서 경고가 뜹니다. macOS에서는 업데이트한 뒤
  캘린더를 처음 읽을 때 키체인 암호를 한 번 더 물을 수 있습니다 — `항상 허용`을
  누르면 그 버전에서는 다시 묻지 않습니다.
...
왜 이렇게 만들었는지는 [DESIGN.md](DESIGN.md)에 따로 적어 두었습니다 — 3구역
시간축의 근거, 긴급도를 여러 채널로 나눈 이유, 대비 계산, 두 플랫폼에서 위젯을
바탕화면에 세워 두는 정반대의 방법, 캘린더를 OAuth 대신 ICS로 간 이유, 그리고
해 보고 걷어낸 것들.
